...
const whiteBackgroundLogos = ["GitHub", "Slack", "Microsoft 365"];

export function IntegrationCard({ name, category, logo }: Props) {
  const needsWhiteBackground = whiteBackgroundLogos.includes(name);
    ...
    <div className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025] p-7 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/40 hover:bg-white/[0.05] hover:shadow-[0_0_50px_rgba(139,92,246,.18)]">
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-5">
          <div
            className={`flex h-16 w-16 items-center justify-center rounded-2xl border transition-transform duration-300 group-hover:scale-110 ${
              needsWhiteBackground
                ? "border-white/20 bg-white"
                : "border-white/10 bg-gradient-to-br from-violet-500/15 via-white/[0.02] to-transparent"
            }`}
          >
            <Image
              src={logo}
